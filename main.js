import * as GlobalHandlers from './handlers/global.js';
import * as ModalHandlers from './handlers/modals.js';
const { open_modal, open_options_modal, close_modal } = ModalHandlers;
...
const editorThemes = {
    "dracula-dark": {
        base: 'vs-dark', // Use standard dark as the base
        inherit: true,   // Inherit other defaults from vs-dark
        rules: [
        ...
        colors: {
            ...
            'editorIndentGuide.activeBackground': '#8be9fd',
            
            // Suggestion Widget (IntelliSense)
    ...
    "cobalt-dark": {
        ...
        inherit: true,
        rules: [
            { token: 'comment', foreground: '007be6', fontStyle: 'italic' }, // Light Blue for comments
            { token: 'keyword', foreground: 'ff9d00', fontStyle: 'bold' }, // Bright Orange for keywords
            { token: 'operator', foreground: 'ff9d00' },
            { token: 'identifier', foreground: 'e1efff' },
            { token: 'string', foreground: '3ad900' }, // Vibrant Green for strings
            { token: 'number', foreground: 'ff628c' }, // Pink/Red for numbers
            { token: 'type', foreground: '80ffbb' },   // Mint for types
            { token: 'class', foreground: '80ffbb' },
            { token: 'function', foreground: 'ffdd00' }, // Yellow for functions
            { token: 'variable', foreground: 'e1efff' },
            { token: 'variable.parameter', foreground: 'fb94ff' },
            { token: 'tag', foreground: '9effff' }, // Cyan for HTML tags
            { token: 'attribute.name', foreground: 'ff9d00' },
            { token: 'delimiter', foreground: 'e1efff' }
        ],
        colors: {
            // Editor UI - The core "Cobalt" look
            'editor.background': '#193549', // The classic dark navy blue
            'editor.foreground': '#e1efff', // White/Light Blue text
            
            // UI Accents
            'editorCursor.foreground': '#ffc600', // Yellow cursor
            'editor.selectionBackground': '#0050a4', // Darker blue selection
            'editor.lineHighlightBackground': '#1f4662', // Slightly lighter blue for current line
            'editorLineNumber.foreground': '#0088ff', // Bright blue line numbers
            'editorIndentGuide.background': '#1f4662',
            'editorIndentGuide.activeBackground': '#ffc600',
            
            // Widget/Popup colors
            'editorSuggestWidget.background': '#122738',
            'editorSuggestWidget.border': '#0088ff',
            'editorSuggestWidget.selectedBackground': '#1f4662',
        }
    },
    "creamy-light": {
        base: 'vs', // Start with standard light base
        inherit: true,
        rules: [
            { token: 'comment', foreground: '93a1a1', fontStyle: 'italic' },
            { token: 'keyword', foreground: '859900', fontStyle: 'bold' }, // Olive Green
            { token: 'identifier', foreground: '586e75' }, // Dark Gray
            { token: 'string', foreground: '2aa198' }, // Cyan/Teal
            { token: 'number', foreground: 'd33682' }, // Magenta
            { token: 'type', foreground: 'b58900' }, // Yellow/Orange
        ...
        colors: {
            // Core UI - Warm, creamy background
            'editor.background': '#fdf6e3', 
            'editor.foreground': '#586e75',
            
            // UI Accents
            'editorCursor.foreground': '#657b83',
            'editor.selectionBackground': '#eee8d5', // Slightly darker beige for selection
    ...
    "clean-light": {
        base: 'vs', // Use standard light base
        inherit: true,
        rules: [
            { token: 'comment', foreground: 'a0a1a7', fontStyle: 'italic' }, // Cool gray
            { token: 'keyword', foreground: '6c3082', fontStyle: 'bold' },   // Deep Violet
            { token: 'operator', foreground: '6c3082' },
            { token: 'identifier', foreground: '383a42' }, // Soft Black (Slate)
            { token: 'string', foreground: '0184bc' },     // Muted Cyan
            { token: 'number', foreground: 'aa0982' },     // Magenta/Pink
            { token: 'type', foreground: '4078f2' },       // Soft Blue
            { token: 'class', foreground: '4078f2' },
            { token: 'function', foreground: '4078f2' },
            { token: 'variable', foreground: 'c18401' },   // Muted Gold
            { token: 'delimiter', foreground: '383a42' }
        ],
        colors: {
            // Editor UI - Pure White
            'editor.background': '#ffffff', 
            'editor.foreground': '#383a42', // Not pitch black, easier on eyes
            
            // UI Accents
            'editorCursor.foreground': '#6c3082', // Violet cursor to match keywords
            'editor.selectionBackground': '#e5e5e6', 
            'editor.lineHighlightBackground': '#fafafa', // Very subtle active line
...
function init() {
    ...
    if (data) {
        ...
        state.editorTheme = parsed.editorTheme || 'cobalt-dark';
...
const update_debug = () => {
    ...
    document.getElementById('state-output').textContent = JSON.stringify({
        ...
        isDraggingNode: state.isDraggingNode?.id || null,
        isResizingNode: state.isResizingNode?.id || null
    }, null, 2);
};

function saveData() {
    const data = {
        nodes: state.nodes.map(({ editor, ...rest }) => rest),
...
/** DOM HELPERS */
const create_monaco_editor = ({node, container, options}) => {
    if (node.hasOwnProperty('editor')) return; // already created
    ...
    // Create the editor with the selected theme
    require(['vs/editor/editor.main'], function() {
        Object.keys(editorThemes).forEach(themeName => monaco.editor.defineTheme(themeName, editorThemes[themeName])); // Import themes
        node.editor = monaco.editor.create(container, options);
    });
};
...
function addNodeData(x, y, w, h, title, desc, type = 'default') {
    const node = { id: GlobalHandlers.generateId(), x, y, w, h, title, desc, type };
...
const process_default_node = async ({targetNode, inputData}) => {
    ...
    // add processing class to connections as well
    connectedOutputNodes.forEach(conn => {
        const connectionEl = document.querySelector(`[data-connection-id="${conn.id}"]`);
        if (connectionEl) connectionEl.classList.add('processing');
    });

    await new Promise(resolve => setTimeout(resolve, 500)); // Delay for visualization purposes
    console.log(`Processing node '${targetNode.title}' with input:`, inputData);

    // Set the input data description
    const inputDescEl = targetNodeEl.querySelector('.node-desc');
    inputDescEl.value = formatDesc(JSON.stringify(inputData));

    // Render resource badges
    GlobalHandlers.render_resource_badges({nodeElement: targetNodeEl, options: {
        show_remove_button: targetNode.type === 'default'
    }});
    ...
    if(operations.length > 0) {
        ...
        operations.forEach(operation => {
            ...
            try {
                ...
                // Remove processing class from connections as well
                connectedOutputNodes.forEach(conn => {
                    const connectionEl = document.querySelector(`[data-connection-id="${conn.id}"]`);
                    if (connectionEl) connectionEl.classList.remove('processing');
                });
            } catch (err) {
        ...
        if (outputData !== undefined) {
            await GlobalHandlers.promise_chain(connectedOutputNodes.map(conn => async () => {
                ...
                if(outputNode && outputNode.type === 'output') {
                    ...
                    // Render resource badges
                    GlobalHandlers.render_resource_badges({nodeElement: outputNodeEl, options: {
                        show_remove_button: false
                    }});
                }

                // Pass output to connected default nodes
                else if(outputNode && (outputNode.type === 'default' || outputNode.type === 'transfer')) {
...
function renderNode(node) {
    ...
    el.dataset.resizable = node.type === 'input' || node.type === 'default' || node.type === 'output';
    ...
    el.innerHTML = `
        ...
        <div class="node-body">
            ...
            node.type === 'output' ? `
                ...
                <div class="resource-badges"><span class="resource-default-message">Output data will appear here.</span></div>
            ` : ''}

            ${// Transfer Node Fields
            node.type === 'transfer' ? `
                <textarea class="node-desc" id="transfer-value" placeholder="Transfer nodes simply pass along unchanged data to any connected nodes." disabled></textarea>
                <div class="resource-badges"><span class="resource-default-message">Incomming data will appear here.</span></div>
            ` : ''}
            ...
            node.type === 'default' ? `
                ...
                <div class="editor-wrapper"><div class="monaco-editor-container" id="default-node-editor"></div></div>
            ` : ''}
    ...
    const editorEl = el?.querySelector('.monaco-editor-container') || null;
    ...
    if (node.type === 'default') { 
        ...
        create_monaco_editor({ node, container: editorEl, options: { value: node.desc } });
        setTimeout(() => {
            node.editor.onDidChangeModelContent(() => {
                state.nodes.find(n => n.id === node.id).desc = node.editor.getValue();
                saveData();
            });
        }, 100);
    }
    ...
    if (node.type === 'input') {
        ...
        el.querySelector('.btn-node-field-apply').onclick = () => {
            ...
            if(name && !isNaN(amount)) {
                ...
                // Re-render and save
                const badge = `<div class="resource-badge">${name} = ${amount} <div class="resource-badge-remove" data-key="${name}">×</div></div>`;
                el.querySelector('.resource-badges').innerHTML += badge;
                saveData();
            }
        }

        // Render resource badges
        GlobalHandlers.render_resource_badges({nodeElement: el});
    }
    ...
    node.type === 'input' && (el.querySelector('.btn-node-run').onclick = async () => {
        ...
        await GlobalHandlers.promise_chain(connectedConns.map(conn => async () => {
            const targetNode = state.nodes.find(n => n.id === conn.to);
            if(targetNode && (targetNode.type === 'default' || targetNode.type === 'transfer')) {
    ...
    el.querySelectorAll('input, textarea, .monaco-editor-container').forEach(inputEl => {
...
function updateTempConnection(e) {
    ...
    const mp = GlobalHandlers.screenToWorld({state, sx: e.clientX, sy: e.clientY});
    const start = GlobalHandlers.getCenter({
...
function renderConnections() {
    ...
    state.connections.forEach(conn => {
        ...
        if (!skip) {
            ...
            // Calculate points
            const c1 = GlobalHandlers.getCenter(n1);
            const c2 = GlobalHandlers.getCenter(n2);
            ...
            // Modify the center point in order to prevent the line from intersecting with other nodes
            const preventIntersection = (cx1, cy1, cx2, cy2) => {
                const padding = 10; // Padding to prevent intersection
                const intersectingNodes = state.nodes.filter(n => {
                    n = document.getElementById(n.id);
                    const rect = n.getBoundingClientRect();
                    console.log('checking intersection with node', n.id, rect);
                    return (
                        rect.left < cx2 + padding &&
                        rect.right > cx1 - padding &&
                        rect.top < cy2 + padding &&
                        rect.bottom > cy1 - padding
                    );
                });
                if (intersectingNodes.length > 0) {
                    // If intersecting, adjust control points
                    cx1 += padding;
                    cx2 -= padding;
                }
            ...
            //({ cx1, cy1, cx2, cy2 } = preventIntersection(cx1, cy1, cx2, cy2));
...
viewport.addEventListener('pointerdown', e => {
    ...
    if(nodeEl) {
        ...
        const wp = GlobalHandlers.screenToWorld({state, sx: e.clientX, sy: e.clientY});
...
window.addEventListener('pointermove', e => {
    ...
    if (state.isResizingNode) {
        ...
        nodeEl.classList.add('is-dragging');
        const wp = GlobalHandlers.screenToWorld({state, sx: mx, sy: my});
...
document.getElementById('add-node').onclick = (e) => {
    const wp = GlobalHandlers.screenToWorld({state, sx: viewport.clientWidth/2, sy: viewport.clientHeight/2});
    open_options_modal({
        element: e.target,
        title: 'Create Node',
        option_btns: [
            ...
            { 
                ...
                classes: 'orange',
                label: 'Transfer Node', 
                onclick: () => addNodeData(wp.x - 75, wp.y - 50, 200, 100, "Transfer Node", "", 'transfer') 
            },
            {
                id: 'add-input-node', 
                classes: 'green',
...
document.getElementById('change-theme').onclick = (e) => {
    open_options_modal({
        element: e.target,
        title: 'Change Editor Theme',
        option_btns: Object.keys(editorThemes).map(themeId => {
            const userFriendlyLabel = themeId
                ?.replace(/-/g, ' (') // Replace dashes with spaces
                ?.replace(/\b\w/g, char => char.toUpperCase()) // Capitalize first letter of each word
                +")" // Ensure closing parenthesis is at the end
            return {
                id: `theme-${themeId}`,
                classes: state.editorTheme === themeId ? 'disabled' : 'blue',
